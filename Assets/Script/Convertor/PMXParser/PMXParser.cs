...
namespace MMD_URP
{
    public class PMXParser
    {
        ...
        public static PMXObject Parse(string ModelPath)
        {
            try
            {
                if (!File.Exists(ModelPath)) 
                    { throw new FileNotFoundException("The model file not found (模型未找到)", ModelPath); }
        ...
        private static void ParseHeader(PMXObject obj)
        {
            ...
            {
                throw new System.FormatException("NOT A *.pmx FILE");
            }
            ...
            header.Encoding = binaryReader.ReadByte() switch
            {
                0 => Encoding.Unicode,
                1 => Encoding.UTF8,
                _ => throw new Exception("Invalid Transformation Format, can't get model information"),
            };
        ...
        private static void ReadVertex(PMXVertex vertex, int additionUVCount, byte boneIndexSize)
        {
            ...
            switch (weightMethod)
            {
                ...
                case PMXVertex.WeightMethod.QDEF:
                    ...
                    break;
                default:
                    throw new System.FormatException("Invalid weight transform type");
            }
        ...
        private static void ReadMaterial(PMXMaterial PMXmaterial, Shader shader, PMXHeader header)
        {
            ...
            PMXmaterial.flag = binaryReader.ReadByte();
            ...
            for (int i = 0; i < morph_offset_count; ++i)
            {
                switch (morph.morph_type)
                {
                    ...
                    case PMXMorph.MorphType.Impulse:
                        ...
                        break;
                    default:
                        throw new System.FormatException("Invalid morph type");
                }
            }
        }
        //Read DisplayFrame
