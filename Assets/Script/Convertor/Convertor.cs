...
namespace MMD_URP
{
    ...
    {
        private static Convertor m_instance;
        public static Convertor Instance
        {
            get 
            {
                if (m_instance == null)
                    m_instance = new Convertor();
                return m_instance;
            }
        }
        ...
        #endregion
        public void Import(string ModelPath, string TexturesPath = null)
        {
            try 
            {
                var extension = CheckModelFormat(ModelPath);
                object extensionType;
                if (Enum.TryParse(typeof(Format), extension, out extensionType)) 
                {
                    throw new Exception($"the imported file {ModelPath} uses the unsupported format: {extension}");
                }

                if ((Format)extensionType == Format.pmd) 
                { }
                ...
                //Create Model Object
                GameObject model = new GameObject(obj.header.Name);

                //Read Comment
                TextAsset ReadMe = new TextAsset(string.Format("ReadMe \n {0} \n {1}", obj.header.Comment, obj.header.CommentEnglish));
            ...
            {
                PhysicsMaterial material = new PhysicsMaterial(string.Format("{0}_r ({1})", model.name, rigidbodyList[i].rigidbodyName));
        ...
        string CheckModelFormat(string path) 
        {
        ...
        //            clip.SetCurve(current_bone, AnimationClip.CurveType.PositionX, curve_x);
        //            clip.SetCurve(current_bone, AnimationClip.CurveType.PositionY, curve_y);
        //            clip.SetCurve(current_bone, AnimationClip.CurveType.PositionZ, curve_z);
